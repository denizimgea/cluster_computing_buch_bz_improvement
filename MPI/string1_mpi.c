...
int main(int argc, char *argv[]) {
  ...
  for (t=2*dt; t<=t_end; t+=dt) {
    ...
    if (C_size>1) {
      /* nach rechts senden */
      if (0==C_rank)
        MPI_Send(&u_new_l[N_l[C_rank]-1], 1, MPI_DOUBLE, C_rank+1,
                 right_copy, MPI_COMM_WORLD);
      if (0<C_rank && C_rank<C_size-1)
        MPI_Sendrecv(&u_new_l[N_l[C_rank]-1], 1, MPI_DOUBLE, C_rank+1,
                     right_copy,
                     &u_new_l[0],             1, MPI_DOUBLE, C_rank-1,
                     right_copy,
                     MPI_COMM_WORLD, &status);
      if (C_rank==C_size-1)
        MPI_Recv(&u_new_l[0], 1, MPI_DOUBLE, C_rank-1, right_copy,
                 MPI_COMM_WORLD, &status);
      /* nach links senden */
      if (0==C_rank)
        MPI_Recv(&u_new_l[N_l[C_rank]], 1, MPI_DOUBLE, C_rank+1,
                 left_copy, MPI_COMM_WORLD, &status);
      if (0<C_rank && C_rank<C_size-1)
        MPI_Sendrecv(&u_new_l[1],           1, MPI_DOUBLE, C_rank-1,
                     left_copy,
                     &u_new_l[N_l[C_rank]], 1, MPI_DOUBLE, C_rank+1,
                     left_copy,
                     MPI_COMM_WORLD, &status);
      if (C_rank==C_size-1)
        MPI_Send(&u_new_l[1], 1, MPI_DOUBLE, C_rank-1, left_copy,
                 MPI_COMM_WORLD);
    }
    /* Felder tauschen */
